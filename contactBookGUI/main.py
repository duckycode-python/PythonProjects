...
def addnew():
    ...
    if isinstance(int(number), int) and len(number) == 9:
        pass
    else:
...
def exportlist():
    ...
    if type == "1":
        ...
        for x in contacts:
            ...
            f.write(line + "\n")
        f.close
        print(f"Succesfully exported contact list to {filename}.txt.")
        back()
    elif type == "2":
        f = open(f"{filename}.json", "w")
        json.dump(contacts, f)
        f.close
        print(f"Succesfully exported contact list to {filename}.json.")
        back()
    else:
        print ("Error!")
        back()
